defmodule SymphonyElixir.StatusDashboard do
    ...
    project_part =
      cond do
        ...
        tracker.kind == "plane" and is_binary(tracker.host) and is_binary(tracker.workspace_slug) and
          ...
          colorize(
            plane_project_url(tracker.host, tracker.workspace_slug, tracker.project_id),
            @ansi_cyan
          )
  ...
  defp plane_project_url(host, workspace_slug, project_id) do
    h = String.trim_trailing(host, "/")
    "#{h}/#{workspace_slug}/projects/#{project_id}/issues"
  end
  ...
  defp humanize_codex_event(:session_completed, _message, payload) do
    session_id = map_value(payload, ["session_id", :session_id])
    ...
    parts = if is_binary(session_id), do: parts ++ ["session=#{session_id}"], else: parts
    parts = if is_integer(total), do: parts ++ ["tokens=#{total}"], else: parts
    Enum.join(parts, " ")
  end

  defp humanize_codex_event(:turn_progress, _message, _payload), do: nil
