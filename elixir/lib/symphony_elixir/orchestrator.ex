defmodule SymphonyElixir.Orchestrator do
    ...
    case Map.get(running, issue_id) do
      ...
      running_entry ->
        updated_running_entry =
          ...
          |> maybe_put_runtime_value(:session_id, runtime_info[:session_id])
    ...
    if is_integer(elapsed_ms) and elapsed_ms > timeout_ms do
      ...
      if input_required_blocker?(running_entry) do
        error =
          blocker_error(
            running_entry,
            "stalled for #{elapsed_ms}ms after agent requested operator input"
          )
  ...
  defp session_id_for_update(_existing, %{payload: %{session_id: session_id}})
  ...
  defp summarize_codex_update(update) do
    %{
      event: update[:event],
      message: summarize_update_message(update),
      timestamp: update[:timestamp]
    }
  end

  defp summarize_update_message(%{payload: %{text: text}}) when is_binary(text) and text != "",
    do: text
  ...
  defp absolute_token_usage_from_payload(payload) when is_map(payload) do
    absolute_paths = [
      ...
      ["tokenUsage", "total"],
      [:tokenUsage, :total],
      # Claude flat usage map
      ["usage"],
      [:usage]
    ]

    direct = if integer_token_map?(payload), do: payload, else: nil

    direct || explicit_map_at_paths(payload, absolute_paths)
  end
